            ...
            WindowEvent::KeyboardInput { event, is_synthetic: false, .. } => {
                ...
                if event.state.is_pressed() {
                    if let Key::Character(ch) = event.key_without_modifiers.as_ref() {
                        window.ime_state = window
                            .ime_state
                            .clone()
                            .map(|ime| {
                                let (pos, area) = ime.cursor_area.unwrap();
                                let pos: LogicalPosition::<f64> = pos.to_logical(1.0);
                                let pos = LogicalPosition { x: pos.x + 10., y: pos.y}.into();
                                    //.unwrap_or(LogicalPosition { x: 0, y: 0 }.into());
                                ime.with_cursor_area(pos, area)
                            });
                        window.window.set_ime_state(window.ime_state.as_ref());
                    }
                    if let Key::Named(NamedKey::Enter) = event.key_without_modifiers.as_ref() {
                        window.ime_state = window
                            .ime_state
                            .clone()
                            .map(|ime| {
                                let (pos, area) = ime.cursor_area.unwrap();
                                let pos = LogicalPosition { x: 0., y: pos.to_logical::<f64>(1.0).y + 20. }.into();
                                ime.with_cursor_area(pos, area)
                            });
                        window.window.set_ime_state(window.ime_state.as_ref());
                    }
